...
The key null-calibration property (`test/q-r04-nullcalibration.test.ts`): on a no-fault,
...
**Scope of that property (honest boundary):** the telemetry is generated by exactly a
linear-factor + OU DGP, so the factor-aware comparison recovers structure the generator
put there. q-r04 demonstrates the fixtures are hard enough to punish stationarity
assumptions and that factor-aware detection is internally consistent with this model
family — it is not independent evidence that factor-aware detection works on
real-cluster telemetry, which requires data from outside the generator's model family
(Tessera's GWDG κ anchoring is the first external tie-down).
